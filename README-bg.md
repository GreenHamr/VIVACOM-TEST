...
Backend-ът слуша локално (напр. `127.0.0.1:8080`). Apache2 стои отпред като **reverse proxy** и препраща заявките за `/ip-inventory/` към C++ сървъра.
...
- Добави същите `ProxyPass` и `ProxyPassReverse` за `/ip-inventory/` към `http://127.0.0.1:8080/ip-inventory/`.
...
Увери се, че backend-ът работи на порт 8080 (или конфигурирания в `config.conf`). След това:
...
**Файлове:** `web/index.php` – форма с редове за IP и тип (IPv4/IPv6); изпраща заявка към API. `web/config.php` – базов URL на API (по подразбиране `http://127.0.0.1:8080`). Промяна чрез env: `IPINVENTORY_API_URL`.
